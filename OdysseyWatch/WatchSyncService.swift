    ...
    private func buildUploadEntries(_ entries: [DailyEntry]) async throws -> [SyncUploadEntry] {
        ...
        for entry in entries {
            let upload = try SyncUploadEntry(
                ...
                latitude: entry.latitude != nil ? try await encryption.encryptDouble(entry.latitude!) : nil,
                longitude: entry.longitude != nil ? try await encryption.encryptDouble(entry.longitude!) : nil,
                city: entry.city != nil ? try await encryption.encrypt(entry.city!) : nil,
                state: entry.state != nil ? try await encryption.encrypt(entry.state!) : nil,
                country: entry.country != nil ? try await encryption.encrypt(entry.country!) : nil,
